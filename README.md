...
- `docs` — как работать с проектом.

## Как работать

См. [docs/README.md](docs/README.md): паспорт и стандарты в трекере, в репозитории их нет. Используй подключённые MCP и API.
...
## Общий дилерский раздел

Переходный буфер на время миграции с закрытого раздела Tilda
(`avgst.ru/dealers/*`), где все дилеры сидели под одним логином. Пока дилер не
зарегистрирован как партнёр, «Общий раздел» — единственное, что он видит:
витрины домов с заводскими ценами, фермы на МЗП, кровельные панели и подборки
материалов.

| Что | Откуда данные |
|---|---|
| Панельно-каркасные и модульные дома | `catalog_projects`, цена — `base_price` завода |
| Фермы на МЗП, кровельные панели | `factory_products`, заводятся в кабинете HQ |
| Материалы для дилеров | `dealer_materials` — ссылки на внешнее облако |
| Запрос расчёта и шеф-монтажа | уходит запросом в мессенджер, не в отдельный ящик |

### Как дилер туда попадает

Общий вход — роль `dealer_guest`, один логин на всех, те же креды, что были в
закрытом разделе Tilda. Учётка заводится при старте API из `DEALER_EMAIL` и
`DEALER_PASSWORD`, пароль меняется в окружении, а не в кабинете.

После входа такой дилер попадает сразу в общий раздел, и в меню у него только
он. Своего кабинета у роли нет: `/api/partner/site`, `/deals`, `/team` и
прочие партнёрские ручки отвечают ему 403, а ручной заход на страницу кабинета
возвращает его в общий раздел.

Партнёр-заглушка «Дилеры AVGST» нужен ровно для одного — чтобы запрос расчёта
было кому адресовать: мессенджер привязывает разговор к партнёру. Доступ к
кабинету он не открывает, это решает роль. Обратная сторона общего входа:
заявки всех дилеров идут одним потоком, различить их можно только по городу и
телефону из формы.

Персональные цены партнёра живут в «Каталоге» и здесь не показываются: общий
раздел одинаков для всех. HQ правит содержимое в разделе «Общий раздел» своего
кабинета, там же прячет позиции переключателем, не удаляя их.

Материалы дилеров — файл в Garage (S3) или внешняя ссылка. Загрузка через
кабинет завода; скачивание только для авторизованных ролей (короткоживущая
signed-ссылка через API). Env: `S3_*` в `.env.example` / compose Dokploy, задача B2B-5.
