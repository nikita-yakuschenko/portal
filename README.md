...
Партнёр ведёт заявку по короткой воронке: «Новая» → «В работе» → «Успех» или
«Отказ» (`site_request_status`). Статус меняется перетаскиванием карточки на
доске или выбором в самой карточке; там же — заметка менеджера (`note`).
Раздел показывает оба вида, доску и таблицу, вид запоминается в адресе
(`?view=table`). Карточка открывается кликом по имени — и на доске, и в
таблице.

Что происходило с заявкой, пишется в `site_request_events`: создание, смены
статуса, заметки, попытки передачи в CRM. Лента показывается в карточке
справа от полей. Отдельная таблица, а не общий `audit_logs`: события видит
партнёр, а аудит-лог служебный и хранит действия по всей платформе.
