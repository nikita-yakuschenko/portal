...
> **Предшественник:** AES-1.0 от 29 июля 2026 года  
> **Изменения в 1.1:** раздел «Git и границы полномочий агента» — запрет
> самостоятельного коммита и push, запрет авторства и соавторства AI  
...
## Git и границы полномочий агента

Право фиксировать и распространять изменения принадлежит человеку. Агент готовит изменения — решение о том, что войдёт в историю репозитория и что уедет в удалённый, принимает владелец.

### Запрет самостоятельного коммита и push

ЗАПРЕЩЕНО любым агентным и AI-системам самостоятельно выполнять:

- `git commit` в любой форме, включая `--amend`, `--fixup` и коммит через сторонние инструменты;
- `git push`, `git push --force` и любую иную отправку в удалённый репозиторий;
- создание, слияние и закрытие pull request;
- `git tag`, публикацию релизов и пакетов.

Агент завершает работу изменениями в рабочем дереве и, при необходимости, подготовленным текстом сообщения коммита. Фиксирует и отправляет их человек.

Единственное исключение — прямое и явное указание владельца выполнить конкретную операцию в конкретный момент. Такое разрешение действует один раз и не распространяется ни на следующие изменения, ни на push, если о нём не сказано отдельно.

### Запрет авторства и соавторства AI

ЗАПРЕЩЕНО указывать авторство или соавторство AI/агента в коммитах, сообщениях коммитов, trailer-ах и метаданных.
...
- подстановка агента в поля `author` и `committer`.

Автор коммита — человек (владелец репозитория / исполнитель), который принимает и фиксирует изменение. Агент не является ни автором, ни соавтором.

### Обнаружение нарушения

Если в истории найден коммит с авторством или соавторством AI, он исправляется до отправки в удалённый репозиторий. Уже отправленные коммиты переписываются только по решению владельца: переписывание общей истории дороже самой пометки.
