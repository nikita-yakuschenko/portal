...
> **Прежняя редакция:** `AVGST_Brandbook_2026.pdf` — тот же документ до переноса в markdown  
> **Связанные документы:** [`AES-1.1.md`](AES-1.1.md)  
...
| Результат действия | `Sonner` toast | успех/отказ мутации, см. §11.5.1 |
...
#### 11.5.1. Канал обратной связи: toast или alert

Граница проходит по природе сообщения, а не по его тональности.

| Природа сообщения | Канал | Примеры |
|---|---|---|
| Событие — реакция на действие пользователя | `Sonner` toast | «Цена и допы сохранены», «Сотрудник добавлен», «Не удалось подключить CRM» |
| Состояние экрана — не зависит от последнего клика | `Alert` в потоке страницы | «Не удалось загрузить сотрудников», «Данные устарели» |
| Постоянное пояснение или предупреждение о режиме | `Alert` / `Badge` / banner | «Кабинет работает в тестовом режиме» |

Правила:

- ОБЯЗАТЕЛЕН один `<Toaster />` в корневом layout; локальные экземпляры ЗАПРЕЩЕНЫ.
- Успех мутации — `toast.success`, отказ мутации — `toast.error`. Оба ЗАПРЕЩЕНО дублировать инлайновым текстом рядом с формой.
- ЗАПРЕЩЕНО отдавать тосту сведения, которые пользователю нужно сохранить или перечитать (пароль, идентификатор, ссылка), без `duration: Infinity`, кнопки закрытия и действия «Скопировать».
- Текст тоста — короткая фраза без точки в конце; подробности идут в `description`.
- Ошибка загрузки данных остаётся на экране `Alert`, потому что она описывает текущее состояние, а не исчезнувшее событие.

### 11.6. Иконографика Product UI

> Единственное задокументированное отступление ADS от канона shadcn: библиотека иконок.
> Во всём остальном §11.1 действует без исключений.

- ОБЯЗАТЕЛЬНО: **Tabler** (`@tabler/icons-react`, `components.json` → `iconLibrary: "tabler"`).
- ОБЯЗАТЕЛЬНО заменять Lucide на Tabler и внутри примитивов registry (`Select`, `DropdownMenu`, `Dialog`, `Sheet`, `Sidebar`, `Checkbox`, `Breadcrumb`, `Spinner`): единый набор важнее нетронутого исходника примитива.
- ЗАПРЕЩЕНО смешивать Lucide и Tabler; `lucide-react` не должен оставаться в импортах Product UI.
- Стиль: линейные outline, дефолтный `stroke={2}`, без emoji и 3D-пиктограмм.
- Базовые размеры — как в shadcn demos (`size-4` в кнопках и пунктах меню, `size-5` в шапках, `size-8`–`size-12` в empty state).
- Геометрию задаёт CSS-класс (`size-4`), а не проп `size` — так примитивы shadcn сохраняют контроль над размером.

#### 11.6.1. Анимация иконок

Анимация ОБЯЗАТЕЛЬНА там, где иконка отражает процесс или переключаемое состояние, и ЗАПРЕЩЕНА как декорация.

| Случай | Иконка | Поведение |
|---|---|---|
| Ожидание ответа | `IconLoader2` | непрерывный `animate-spin` |
| Выполняется синхронизация / обновление | `IconRefresh` | `animate-spin` только на время процесса |
| Раскрывающийся триггер | `IconChevronDown` | поворот на 180° по `data-state="open"` |
| Переключение темы | `IconSun` / `IconMoon` | кроссфейд + `rotate` по классу `dark` |
| Появление статуса | `IconCircleCheck` и т.п. | краткое `animate-in fade-in` при смене состояния |

Правила:

- Длительность — токены motion (150–300 мс), кроме бесконечных спиннеров.
- При `prefers-reduced-motion: reduce` переходы и повороты ОБЯЗАТЕЛЬНО отключаются (`motion-reduce:animate-none`); вращение индикатора загрузки сохраняется, так как оно единственный носитель смысла.
- ЗАПРЕЩЕНЫ hover-анимации иконок ради «оживления» интерфейса.
...
`tw-animate-css` ОБЯЗАТЕЛЕН: без него утилиты `animate-in` / `fade-in` / `zoom-in` / `slide-in-from-*`, зашитые в примитивы registry, не генерируются, и поповеры появляются рывком.
...
.dark {
  ...
  /* Шаг от --card (0.26) сохраняем каноническим (~0.06), иначе поверхности сливаются */
...
8. Мигрировать иконки затронутого экрана на Tabler (§11.6), включая примитивы registry.
...
2. Tabler как единственный набор иконок; вывод `lucide-react` из зависимостей.
...
- [ ] Иконки Tabler; импортов `lucide-react` не осталось.
- [ ] Анимация иконок только для процессов и состояний (§11.6.1).
- [ ] Есть loading / empty / error.
- [ ] Результат мутации приходит тостом, а не инлайновым текстом (§11.5.1).
...
- Иконки: **Tabler** вместо Lucide как единственное задокументированное отступление от канона shadcn; добавлен § анимации иконок состояний (§11.6.1).
- В канонический скелет темы добавлен `@import "tw-animate-css"` — без него анимации появления поповеров из registry не работают.
- Введён §11.5.1: результат действия пользователя доставляется тостом `Sonner`, состояние экрана — `Alert`.
