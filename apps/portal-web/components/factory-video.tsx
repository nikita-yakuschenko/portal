...
type FactoryVideoProps = {
  /** Фото-плакат для варианта poster */
  src?: string;
  alt?: string;
  /**
   * poster — фото + карточка «Смотреть» (лендинг / десктоп)
   * thumb — только превью ролика с play (мобилка сайта партнёра)
   * responsive — thumb на мобилке, poster на md+
   */
  variant?: "poster" | "thumb" | "responsive";
  className?: string;
};
...
function ThumbPlayButton({ onClick }: { onClick: () => void }) {
  return (
    <button
      type="button"
      onClick={onClick}
      ...
      aria-label="Смотреть видео с производства"
    >
      <img
        src={VIDEO_THUMB}
        alt="Видео с производства Авангард Строй"
    ...
    <div className="group relative mx-auto w-full max-w-md overflow-hidden rounded-xl border border-border bg-card shadow-sm md:mx-0 md:max-w-none">
      ...
      >
        <span className="relative h-[4.25rem] w-[6.5rem] shrink-0 overflow-hidden rounded-lg sm:h-[5rem] sm:w-[7.5rem]">
          <img src={VIDEO_THUMB} alt="" className="h-full w-full object-cover" />
          <img
            src={PLAY_ICON}
            alt=""
            className="absolute top-1/2 left-1/2 h-8 w-8 -translate-x-1/2 -translate-y-1/2 drop-shadow-sm sm:h-9 sm:w-9"
          />
        </span>
...
export function FactoryVideo({
  ...
  return (
    <div className={cn(className)}>
      {variant === "thumb" ? (
        <ThumbPlayButton onClick={() => setOpen(true)} />
      ) : null}

      {variant === "poster" ? (
        <PosterPlayCard src={src} alt={alt} onClick={() => setOpen(true)} />
      ) : null}
      ...
      <VideoModal open={open} onClose={() => setOpen(false)} titleId={titleId} />
    </div>
  );
}
